...
resource "aws_instance" "pwn_drop-redirector" {
  ...
  user_data = <<EOF
              ...
              echo "ExecStart=/usr/bin/sudo /usr/bin/socat TCP4-LISTEN:80,fork,reuseaddr TCP4:${var.pwn_drop_public_ip}:443" >> /etc/systemd/system/redirector.service 
              echo "" >> /etc/systemd/system/redirector.service 
